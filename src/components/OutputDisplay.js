...
const OutputDisplay = ({ dynamicBgd, dynamicColor, celcius }) => {
  let info;
  if (celcius <= 0 && celcius > -5)
    info = (
      <div>
        {" "}
  ...
  if (celcius <= -39 && celcius > -41)
    info = (
      <div>
        {" "}
        <p>At -40, celcius and farenheit are the same! </p>
      </div>
    );
  if (celcius <= -59 && celcius > -79)
    info = (
      <div>
        {" "}
  ...
  if (celcius >= 450 && celcius < 500)
    info = (
      <div>
        <FaPizzaSlice /> <span>Ideal for cooking pizza!</span>
      </div>
    );
  if (celcius >= 660 && celcius < 961)
    info = (
      <div>
